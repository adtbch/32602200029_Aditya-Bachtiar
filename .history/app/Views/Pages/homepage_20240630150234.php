...
$bulan = date('F');
$tahun = date('Y');
$jumlah_hari = cal_days_in_month(CAL_GREGORIAN, date('n'), $tahun);
$tanggal_tugas = array_column($tugas, 'tanggal_deadline');
$days_of_week = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];

// Daftar warna untuk setiap tugas (dapat disesuaikan)
$colors = ['#71A430', '#5BC0EB', '#FDE74C', '#9BC53D', '#F45B69'];

// Generate array untuk menentukan warna untuk setiap tanggal tugas
$date_color_map = [];
foreach ($tanggal_tugas as $index => $tanggal) {
    $date_color_map[$tanggal] = $colors[$index % count($colors)];
}
?>
...
<div class="col-md-4 task-list">
    ...
    <?php foreach ($tugas as $index => $item): ?>
        <div class="task-item" style="border-left: 5px solid <?= $colors[$index % count($colors)] ?>;">
            <h4><?= htmlspecialchars($item['judul']) ?></h4>
            <p><?= htmlspecialchars($item['deskripsi']) ?></p>
            <p class="text-muted">Deadline: <?= htmlspecialchars($item['tanggal_deadline']) ?></p>
            <p class="text-muted">Warna: <?= $colors[$index % count($colors)] ?></p>
        </div>
    <?php endforeach; ?>
</div>
<div class="col-md-8 calendar">
    <div class="month"><?= $bulan . ' ' . $tahun ?></div>
    <table>
        <thead>
            <tr>
        ...
        <tbody>
            <tr>
                ...
                $first_day_of_month = date('w', strtotime("$tahun-$bulan-01"));
                ...
                <?php for ($day = 1; $day <= $jumlah_hari; $day++):
                    $current_date = $tahun . '-' . date('m') . '-' . str_pad($day, 2, '0', STR_PAD_LEFT);
