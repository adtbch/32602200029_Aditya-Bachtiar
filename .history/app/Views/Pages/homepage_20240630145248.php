...
<style>
    ...
    .task-date {
        background-color: #71A430; /* Warna latar belakang umum untuk tanggal tugas */
        color: white;
        font-weight: bold;
    }

    /* Warna khusus untuk setiap tugas */
    .task-1 {
        background-color: #5BC0EB; /* Biru */
    }

    .task-2 {
        background-color: #FDE74C; /* Kuning */
    }

    .task-3 {
        background-color: #9BC53D; /* Hijau */
    }

    .task-4 {
        background-color: #F45B69; /* Merah */
    }
</style>
...
<?php for ($day = 1; $day <= $jumlah_hari; $day++): 
    ...
    $task_index = array_search($current_date, $tanggal_tugas);
    $task_color_class = $is_task_date ? 'task-date task-' . ($task_index % 4 + 1) : '';
    ?>
    <td class="<?= $task_color_class ?>">
        <?= $day ?>
    </td>
    <?php
