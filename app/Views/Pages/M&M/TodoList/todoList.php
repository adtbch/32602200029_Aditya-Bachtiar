            ...
            <div class="col-12 col-md-6">
                <div class="card bg-primary rounded-4 p-4 mb-3">
                    <div class="d-flex justify-content-between align-items-center">
                        <h4 class="text-white"><?= esc($item['catatan']) ?></h4>
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" id="priorityUser_<?= esc($item['id_todolistuser']) ?>" <?= $item['priority'] ? 'checked' : '' ?> onchange="updatePriorityUser(<?= esc($item['id_todolistuser']) ?>)">
                            <label class="form-check-label text-white" for="priorityUser_<?= esc($item['id_todolistuser']) ?>">Priority</label>
                        </div>
                    </div>
                    <div class="d-flex justify-content-between align-items-center mt-3">
                        <h6 class="text-white"><?= esc($item['keterangan']) ?></h6>
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" id="selesaiUser_<?= esc($item['id_todolistuser']) ?>" <?= $item['selesai'] ? 'checked' : '' ?> onchange="updateSelesaiUser(<?= esc($item['id_todolistuser']) ?>)">
                            <label class="form-check-label text-white" for="selesaiUser_<?= esc($item['id_todolistuser']) ?>">Selesai</label>
                        </div>
                    </div>
                    <div class="mt-3 d-flex justify-content-end">
                        <form action="#" class="editTodolistuserForm" method="get" style="display:inline;">
                        <input type="hidden" name="id_todolistuser" value="<?= esc($item['id_todolistuser']) ?>">
